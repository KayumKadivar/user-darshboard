...
function renderPlantNav(activePageId) {
    ...
    const navGroups = [
        {
            items: [
                { id: 'technician_logbook', label: 'Logs', icon: 'ph-list-dashes', href: 'cms_logbook.html' }
            ]
        },
        {
            dropdown: true,
            ...
            items: [
                { id: 'remark_reports', label: 'Remark Report', icon: 'ph-note', href: 'cms_remark_reports.html' },
                { id: 'tagwise_filter', label: 'Tag Wise Filter', icon: 'ph-funnel', href: 'cms_tagwise_filter.html' },
                { id: 'monthly_report', label: 'Monthly Report', icon: 'ph-calendar-blank', href: 'cms_monthly_report.html' },
                { id: 'datewise_report', label: 'Date Wise Report', icon: 'ph-calendar-dots', href: 'cms_datewise_report.html' },
                { id: 'namewise_report', label: 'Name Wise Report', icon: 'ph-user-list', href: 'cms_namewise_report.html' }
            ]
        },
        {
            items: [
                { id: 'ot_hours', label: 'OT Hrs', icon: 'ph-timer', href: 'cms_ot_hr.html' },
                { id: 'helper', label: 'Helper', icon: 'ph-users', href: 'cms_helper.html' }
            ]
        },
        {
            isTools: true,
            items: [
                { id: 'ot_statement', label: 'O.T.', icon: 'ph-clock-user', href: 'cms_ot.html', class: 'btn-ot-sm' },
    ...
    let groupsHtml = navGroups.filter(g => !g.isTools).map((group, gi) => {
        ...
        html += group.items.map(item => {
            ...
            return `
                <button type="button" onclick="window.location.href='${href}'" class="${pillCls}">
    ...
    navGroups.filter(g => g.dropdown).forEach(group => {
        ...
        panelHtml += (group.items || []).map(child => {
            ...
            return `
                <button type="button" onclick="window.location.href='${href}'" class="pn-dropdown-item ${childActive ? 'is-active' : ''}" style="width:100%; text-align:left; border:none; background:none; cursor:pointer;">
